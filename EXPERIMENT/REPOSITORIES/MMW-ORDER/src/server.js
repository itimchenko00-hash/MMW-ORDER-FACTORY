...
import {createOrder,listOrders,getOrderByCredentials,listAllOrders,updateOrderStatus,orderStatuses} from './db.js';
...
app.post('/api/order-access',async(q,r)=>{try{const ip=q.ip||q.socket.remoteAddress||'unknown',now=Date.now(),a=accessAttempts.get(ip)||[],recent=a.filter(t=>now-t<WINDOW);if(recent.length>=MAX_ATTEMPTS)return r.status(429).json({error:'Слишком много попыток. Повторите через 10 минут.'});recent.push(now);accessAttempts.set(ip,recent);const id=String(q.body?.id||'').trim().toUpperCase(),code=String(q.body?.code||'').trim();if(!/^MMW-\d{8}-\d{5}$/.test(id))return r.status(400).json({error:'Введите полный номер заявки в формате MMW-YYYYMMDD-12345.'});if(!/^\d{5}$/.test(code))return r.status(400).json({error:'Введите ровно 5 цифр кода доступа.'});const order=await getOrderByCredentials(id,code);if(!order)return r.status(404).json({error:'Заявка или код доступа не найдены.'});accessAttempts.delete(ip);r.json({order});}catch(e){console.error(e);r.status(500).json({error:'Не удалось открыть заявку.'});}});
app.get('/api/orders/:id/pdf',async(q,r)=>{try{const id=String(q.params.id||'').trim().toUpperCase(),code=String(q.query.code||'').trim();const o=await getOrderByCredentials(id,code);if(!o)return r.status(404).json({error:'Заявка или код доступа не найдены.'});const pdf=await orderPdf(o);r.setHeader('Content-Type','application/pdf');r.setHeader('Content-Disposition',`attachment; filename="${o.id}.pdf"`);r.send(pdf);}catch(e){console.error(e);r.status(500).json({error:'Не удалось сформировать PDF.'});}});
app.post('/api/orders',async(q,r)=>{try{const p=q.body||{};if(!p.customerName?.trim()||!p.phone?.trim()||!p.email?.trim()||!Array.isArray(p.items)||!p.items.length)return r.status(400).json({error:'Заполните имя, телефон, email и добавьте позицию.'});if(!/^\S+@\S+\.\S+$/.test(p.email))return r.status(400).json({error:'Проверьте email.'});const o=await createOrder({customerName:p.customerName.trim().slice(0,120),phone:p.phone.trim().slice(0,40),email:p.email.trim().slice(0,160),company:String(p.company||'').trim().slice(0,160),projectType:String(p.projectType||'').trim().slice(0,100),address:String(p.address||'').trim().slice(0,300),comment:String(p.comment||'').trim().slice(0,2000),items:p.items});const notifications=await notifyOrder(o);const {accessToken,...publicOrder}=o;r.status(201).json({order:publicOrder,accessCode:o.accessCode,notifications});}catch(e){console.error(e);r.status(500).json({error:'Не удалось создать заявку.'});}});
