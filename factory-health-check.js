...
const ROOT=__dirname;const COMPANY=path.join(ROOT,'ЭТАЛОН-03/MMW-COMPANY');const SITE=path.join(COMPANY,'site');const ASSETS=path.join(ROOT,'ASSETS');
const routes={
...
if(fs.existsSync(path.join(ROOT,'assets-server-bootstrap.js')))console.log('[INFO] historical bootstrap file exists but is not used by npm start');
