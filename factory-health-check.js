...
for(const file of walk(srcRoot)){
 ...
 if(external){console.error(`[WARN] active source ${relFile}: Unsplash references=${external}`);seenExternal.push(relFile)}
 for(const rel of assetRefs(text)){if(!fs.existsSync(path.join(ASSET_ROOT,rel))){console.error(`[FAIL] active source ${relFile}: missing asset /assets/${rel}`);missingAssets.push(`${relFile}: /assets/${rel}`);failed=true}}
 const owner='MMW-COMPANY';checkOwnedAssets(owner,relFile,text);
}
...
console.log(`[SUMMARY] active-hooks=${ACTIVE_HOOKS.length} external-unsplash-findings=${seenExternal.length} missing-assets=${missingAssets.length} cross-project-assets=${crossProject.length}`);
