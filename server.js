...
app.use((req,res,next)=>{res.set('Cache-Control','no-store,no-cache,must-revalidate,proxy-revalidate,max-age=0');res.set('Pragma','no-cache');res.set('Expires','0');res.set('X-ALADIN-SOURCE','FACTORY-CANONICAL');res.set('X-ALADIN-REPO','itimchenko00-hash/MMW-ORDER-FACTORY');res.set('X-ALADIN-BRANCH','aladin-six-blocks-working');next()});
