...
function sendHtml(file,res,route){try{let html=fs.readFileSync(path.join(site,file),'utf8');html=cleanText(html);html=stripInternalProjectBlocks(html,route);html=normalizeProjectEconomics(html,route);const project=projectFor(route);if(project!=='MMW-COMPANY')html=mediaTransform(html,project);html=injectStyle(html);res.type('html').send(html)}catch(e){console.error(e);res.status(404).sendFile(path.join(site,'404.html'))}}
