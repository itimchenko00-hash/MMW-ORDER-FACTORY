...
app.use((req,res,next)=>{res.set('Cache-Control','no-store,no-cache,must-revalidate,proxy-revalidate,max-age=0');res.set('Pragma','no-cache');res.set('Expires','0');res.set('X-ALADIN-SOURCE','FACTORY-CANONICAL');res.set('X-ALADIN-REPO','itimchenko00-hash/MMW-ORDER-FACTORY');res.set('X-ALADIN-BRANCH','aladin-six-blocks-working');res.set('X-ALADIN-BUILD','ALADIN-SCANDINAVIAN-2026-09-04');next()});
app.get('/aladin',(req,res)=>{let html=fs.readFileSync(ALADIN,'utf8');const urban=fs.readFileSync(URBAN_OVERRIDE,'utf8');const scandinavian=fs.readFileSync(SCANDINAVIAN_OVERRIDE,'utf8');html=html.replace('</body>','<script>'+urban+'\n'+scandinavian+'</script></body>');res.type('html').send(html)});
