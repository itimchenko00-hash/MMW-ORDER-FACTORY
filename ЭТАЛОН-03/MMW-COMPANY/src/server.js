...
function financeTransform(html,project){
  ...
  const block=`<section class=\"mmw-finance\"><div class=\"mmw-finance-head\"><small>PRELIMINARY / CONCEPT ECONOMICS</small><h2>Финансовый контур проекта</h2><p>Демонстрационная рабочая модель MMW-COMPANY. Проект не является действующим объектом или инвестиционным предложением; показатели требуют проверки исходных данных, рынка и технических условий.</p></div><div class=\"mmw-finance-grid\"><div><b>PACKAGE READINESS</b><strong>${d.readiness}</strong><span>ориентировочная зрелость проектного пакета</span></div><div><b>PRELIMINARY CAPEX</b><strong>${d.capex}</strong><span>концептуальный диапазон для дальнейшей проверки</span></div><div><b>MODEL VALUE</b><strong>${d.value}</strong><span>демонстрационный показатель, не подтверждённая выручка</span></div></div><div class=\"mmw-finance-focus\"><b>NEXT FINANCIAL WORK</b><p>${d.focus}</p></div><div class=\"mmw-finance-chain\"><span>LAND</span><i>→</i><span>CAPEX</span><i>→</i><span>OPEX</span><i>→</i><span>REVENUE</span><i>→</i><span>EBITDA</span><i>→</i><span>ROI / IRR</span><i>→</i><span>PAYBACK</span></div></section>`;
